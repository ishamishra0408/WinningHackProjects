...
layers and the 35 audit tasks, then improved by three advisor passes from
...
| FR-2 | Score any finished repo /5 per scope, running every deterministic probe **before** any human rubric | all 35 tasks, [evaluator/](evaluator/README.md) phases 0–5 | — |
...
| FR-5 | At t=0, produce an 18-row design card where every row returns one of **measured / ABSENT / UNEVALUABLE** | [designer/](designer/README.md) | m08: extend the three states to audit-time too (NFR-7) |
...
| FR-10 *(new, q05/NFR-11)* | Calibrate the gate against known-good repos: run the full pre-submit gate on 2–3 audited winners from [research/winners-top3.md](research/winners-top3.md) before trusting any cap on our own project | evaluator/audits/ (currently empty) | this is also the only way to measure probe false-positive rates |
...
| NFR-9 *(new, m06)* | Threshold provenance | Every numeric constant states the n and tolerance it implies, or is labelled **convention** — "a bar that cannot state them is a convention wearing a measurement." Today `<10 min`, `≥80%`, `<20%`, `≥8 hours`, `±20%` state none. Proposal: add a provenance note per threshold row, contract-first | metric-design m06 |
