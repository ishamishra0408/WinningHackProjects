...
<!-- checks: F-7 F-8 F-10 F-13 F-9 F-11 -->
...
- [ ] 🔒 `F-10` Lockfile integrity — exits 0; `npm ci` / `pip-compile --generate-hashes` / `go mod
      verify` / `cargo --locked`
- [ ] 🔒 `F-13` Runs somewhere other than here, from source alone — lockfile **and** pinned runtime
      **and** a documented start command, with no absolute path or hardcoded localhost in own
      source
...
- [ ] `F-14` Built mass against the hours that existed — ≤150 own LOC per available person-hour
<!-- /checks -->

## 6 · Demo honesty

<!-- checks: V-2 V-7 V-8 V-9 V-10 -->
- [ ] 🔒 `V-2` Evidence each of V-1's criteria in the project — ≥75% of V-1 criteria evidenced in
      source
- [ ] 🔒 `V-7` Prior-art / differentiation — no off-the-shelf equivalent
...
- [ ] 🔒 `U-13` Naive-user walkthrough — 3 of 3 runs say the payoff is reachable
<!-- /checks -->
...
Not fixable at this point: `F-2c`, `F-3` (history), `F-6` (comprehension), and
`V-2` if the build never did what the event's criteria asked for.
