...
| invariant 2 | V-7's confirmation is made by someone who is not one of the project's authors |
...
| V-2 | Evidence each of V-1's criteria in the project | auto, against V-1's criteria | ripgrep over own source | v2_criteria_evidence.sh | criteria-evidence.json — per criterion, the files and lines that satisfy it | ≥75% of V-1 criteria evidenced in source | 2/5 when criteria_source=published — advisory otherwise | 
| V-7 | Prior-art / differentiation | agent search + 1 human confirm | GitHub + registry search | v7_prior_art.sh | priorart.json — the closest existing tools with links, or an explicit none | no off-the-shelf equivalent | 2/5 | 
| V-8 | Reproduce their claimed number | auto, on a command the caller supplies | the project's own eval command | v8_reproduce_claim.sh | claim-reproduction.json — command, exit code, output, matched figure | within ±20% of claim | 3/5 — if no claim made, record claim: none | 
...
|---|---|---|
| raters: 2 + agreement | V-7 | single-rater rubric is unfalsifiable; report raw agreement, escalate to a third on disagreement |
| runs: 2, deterministic: bool | V-8 | re-run it; differing results mean the probe is flaky, not that the project passed |
| time_box_min | V-7 | unbounded search silently becomes the whole review |
| cited_at: file:line | V-2, V-9 | a number with no citation cannot be checked, only believed — every matched figure ships the file and line it was read from |
...
| 2 | V-9, V-10 (parallel, auto) | no reachable demo artifact → capped at 2/5 |
| 3 | V-2, against V-1's criteria | below its bar → capped at 2/5 **only if `criteria_source=published`**, advisory otherwise |
| 4 | V-8, on the command the caller supplied | claim unreproducible → capped at 3/5 |
| 5 | V-7, agent search confirmed by a human | an off-the-shelf equivalent → capped at 2/5 · otherwise final score |
